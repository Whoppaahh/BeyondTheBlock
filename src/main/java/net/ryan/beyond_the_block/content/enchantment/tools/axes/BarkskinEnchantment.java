package net.ryan.beyond_the_block.content.enchantment.tools.axes;
...
import static net.ryan.beyond_the_block.content.enchantment.MyEnchantmentHelper.isLog;
...
public class BarkskinEnchantment extends Enchantment {
    ...
    private void stripTreeAndDrop(World world, BlockPos start, Block logBlock, PlayerEntity player, ItemStack tool, boolean dropNormalLogs, boolean noDamage) {
        ...
        if (!collectedDrops.isEmpty()) {
            switch (dropMode) {
                case NORMAL -> {
                    // Default behaviour: drop all collected items normally
