package net.ryan.beyond_the_block.content.entity.villager.guard;
...
import net.ryan.beyond_the_block.content.entity.villager.guard.goals.*;
...
public class GuardEntity extends PathAwareEntity implements CrossbowUser, RangedAttackMob, Angerable, InventoryChangedListener, InteractionObserver {
    ...
    private static final TrackedData<Boolean> CHARGING_STATE = DataTracker.registerData(GuardEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    private static final UniformIntProvider angerTime = TimeHelper.betweenSeconds(20, 60);
    ...
    public SimpleInventory guardInventory = new SimpleInventory(8);
    ...
    private boolean clientPoisoned, clientWithered, clientFrozen;

    public void setClientSideEffectFlags(boolean poison, boolean wither, boolean frozen) {
        this.clientPoisoned = poison;
        this.clientWithered = wither;
        this.clientFrozen = frozen;
    }

    // Getters for use in rendering
    public boolean hasPoisonClient() {
        return this.clientPoisoned;
    }
    public boolean hasWitherClient() {
        return this.clientWithered;
    }
    public boolean hasFreezeClient() {
        return this.clientFrozen;
    }
    ...
    protected void onStatusEffectRemoved(StatusEffectInstance effect) {
        ...
        if(!this.getWorld().isClient){
            Objects.requireNonNull(this.getServer()).execute(() -> GuardStatusSync.syncGuardStatus((ServerPlayerEntity) this.getWorld().getPlayerByUuid(getOwnerId()), this));
        }
    }
    ...
    public ItemStack getEquippedStack(EquipmentSlot slot) {
        return switch (slot) {
            case HEAD -> this.guardInventory.getStack(0);
            case CHEST -> this.guardInventory.getStack(1);
            case LEGS -> this.guardInventory.getStack(2);
            case FEET -> this.guardInventory.getStack(3);
            case OFFHAND -> this.guardInventory.getStack(4);
            case MAINHAND -> this.guardInventory.getStack(5);
        };
    }
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData dataTracker, @Nullable NbtCompound entityNbt) {
        ...
        if (this.getRandom().nextFloat() < 0.2F) {
            HorseEntity  horse = EntityType.HORSE.create(world.toServerWorld());

            if (horse != null) {
                horse.refreshPositionAndAngles(this.getX(), this.getY(), this.getZ(), this.getYaw(), 0.0F);
                horse.setTame(true);
                horse.setOwnerUuid(this.uuid);
                horse.setPersistent();
    ...
    public void setOwnerId(@Nullable UUID ownerId) {
        this.dataTracker.set(OWNER_UNIQUE_ID, Optional.ofNullable(ownerId));
    }
    ...
    public ItemStack eatFood(World world, ItemStack stack) {
        if (stack.isFood()) {
            this.heal(stack.getItem().getFoodComponent() != null ? stack.getItem().getFoodComponent().getHunger() : 0);
        }
    ...
    public void setCurrentHand(Hand hand) {
        ...
        if (itemstack.getItem() == Items.SHIELD) { // See above
            ...
            Objects.requireNonNull(modifiableAttributeInstance).removeModifier(USE_ITEM_SPEED_PENALTY);
    ...
    public void stopUsingItem() {
        super.stopUsingItem();
        if (Objects.requireNonNull(this.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED)).hasModifier(USE_ITEM_SPEED_PENALTY))
            Objects.requireNonNull(this.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED)).removeModifier(USE_ITEM_SPEED_PENALTY);
    }
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(CHARGING_STATE, false);
    ...
    public boolean isCharging() {
        return this.dataTracker.get(CHARGING_STATE);
    }

    public void setChargingCrossbow(boolean charging) {
        this.dataTracker.set(CHARGING_STATE, charging);
    }
    ...
    public List<ItemStack> getStacksFromLootTable(EquipmentSlot slot) {
        if (EQUIPMENT_SLOT_ITEMS.containsKey(slot)) {
            LootTable loot = Objects.requireNonNull(getWorld().getServer()).getLootManager().getTable(EQUIPMENT_SLOT_ITEMS.get(slot));
    ...
    public void attack(LivingEntity target, float pullProgress) {
        ...
        if (this.getMainHandStack().getItem() instanceof CrossbowItem crossbow) {
            Hand hand = BeyondTheBlock.getHandWith(this, item -> item instanceof CrossbowItem);
            ItemStack stack = this.getStackInHand(hand);

            if (!CrossbowItem.isCharged(stack)) {
                ItemStack ammo = this.getProjectileType(stack);
            ...
            BeyondTheBlock.LOGGER.info("Firing crossbow from attack(...) method");
            CrossbowAccessor.callShootAll(this.getWorld(), this, hand, stack, 2.0F, 1.0F);
        }
        ...
        if (damage >= 0.0F) {
            ...
            for (int i = 0; i < this.guardInventory.size(); ++i) {
                ...
                if ((!damageSource.isFire() || !itemstack.getItem().isFireproof()) && itemstack.getItem() instanceof ArmorItem) {
                    int j = i;
                    itemstack.damage((int) damage, this, (guard) -> guard.sendEquipmentBreakStatus(EquipmentSlot.fromTypeIndex(EquipmentSlot.Type.ARMOR, j)));
                }
            }
        }
    }
