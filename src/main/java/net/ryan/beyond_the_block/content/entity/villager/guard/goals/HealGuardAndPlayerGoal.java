package net.ryan.beyond_the_block.content.entity.villager.guard.goals;
    ...
    public boolean canStart() {
        ...
        if (!list.isEmpty()) {
            for (LivingEntity mob : list) {
                if (mob != null) {
                    if (mob instanceof VillagerEntity && mob.isAlive() && mob.getHealth() < mob.getMaxHealth()
                            && mob != healer || mob.getType() == GUARD_VILLAGER && mob.isAlive() && mob.getHealth() < mob.getMaxHealth()
                            || mob instanceof PlayerEntity && mob.hasStatusEffect(StatusEffects.HERO_OF_THE_VILLAGE)
                            && !((PlayerEntity) mob).getAbilities().creativeMode && mob.getHealth() < mob.getMaxHealth()) {
                        this.mob = mob;
                        return true;
                    }
                }
