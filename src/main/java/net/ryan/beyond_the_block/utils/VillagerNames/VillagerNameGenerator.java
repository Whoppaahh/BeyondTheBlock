...
public class VillagerNameGenerator {
    ...
    public static String pickName(ModConfig.VillagerNamesConfig.GenderMode genderMode) {
        List<String> pool = new ArrayList<>();
        if (genderMode == ModConfig.VillagerNamesConfig.GenderMode.MALE || genderMode == ModConfig.VillagerNamesConfig.GenderMode.BOTH)
            pool.addAll(VillagerNameLoader.getMaleNames());
        if (genderMode == ModConfig.VillagerNamesConfig.GenderMode.FEMALE || genderMode == ModConfig.VillagerNamesConfig.GenderMode.BOTH)
            pool.addAll(VillagerNameLoader.getFemaleNames());

        if (pool.isEmpty()) return "Alex"; // fallback

        return pool.get(RANDOM.nextInt(pool.size()));
    }

    public static List<String> pickNamesStartingWith(char letter, ModConfig.VillagerNamesConfig.GenderMode genderMode) {
        List<String> pool = new ArrayList<>();
        if (genderMode == ModConfig.VillagerNamesConfig.GenderMode.MALE || genderMode == ModConfig.VillagerNamesConfig.GenderMode.BOTH)
            pool.addAll(VillagerNameLoader.getMaleNames());
        if (genderMode == ModConfig.VillagerNamesConfig.GenderMode.FEMALE || genderMode == ModConfig.VillagerNamesConfig.GenderMode.BOTH)
